...
class BaseShader:
    ...
    def __init__(self,
        ...
        self.res = res if res is not None else (1280, 720) #размеры окна
        self.resf = ts.vec2(*self.res) #вещественные размеры
        self.pixels = ti.Vector.field(3, dtype=ti.f32, shape=self.res)#координаты цветов
        self.gamma = gamma

    @ti.kernel
    #функция которая проходит по координатам и обновляет цвета в зависемости от закона "main_image"
    def render(self, t: ti.f32):
        for fragCoord in ti.grouped(self.pixels):
            col = self.main_image(fragCoord, t)
            if self.gamma > 0.0:
